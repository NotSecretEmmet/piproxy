...
echo "=== Disabling the default 3proxy service ==="
...
sudo install -m755 ./startproxy.sh "$BASE_DIR/startproxy.sh"
...
echo "=== Installing modem Python scripts ==="
for script in reset_ip.py modem_status.py; do
    sudo cp "./$script" "$BASE_DIR/$script"
...
[Unit]
Description=PiProxy - 3proxy with modem support
...
echo "3proxy is running and will auto-start on boot."
echo "======================================================"
